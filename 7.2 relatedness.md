...
```{r}
library(related)
data <- read.table("bluemt-isl_related.ped", header=F, stringsAsFactors=F)
...
write.table(anchor_relatedness, file = "~/Desktop/anchorRel.txt")
mean(anchor_relatedness$wang)

boxplot(anchor_relatedness$wang)

#do the above for each of the populations in the comparison 


#add population name to individuals relatedness data frames:
